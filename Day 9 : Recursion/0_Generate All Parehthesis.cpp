...
public:
    void helper(vector<string>& ans, int op, int cp, int n, string& s) {
        ...
        if (op < n) {
            s.push_back('('); // Add '('
            helper(ans, op + 1, cp, n, s); // Recurse
        ...
        if (cp < op) {
            s.push_back(')'); // Add ')'
            helper(ans, op, cp + 1, n, s); // Recurse
            s.pop_back(); // Backtrack
        }
    }
    ...
    vector<string> generateParenthesis(int n) {
        vector<string> ans;
        // Reserve space for the result vector to avoid reallocations
        ans.reserve(1 << n); // Reserve space for 2^n combinations (upper bound)
